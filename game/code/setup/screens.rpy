...
style window:
    ...
    background Image("images/Interface/Textbox.png", xalign=0.5, yalign=1.0)
#
style namebox:
    xpos gui.name_xpos
    ...
    background Frame("images/Interface/namebox.png", gui.namebox_borders, tile=gui.namebox_tile, xalign=gui.name_xalign)
...
screen qq_menu():
    ...
    imagemap:
        auto "images/Interface/quick_menu_%s.png" yalign 0.99 xalign 0.92
        alpha False

        hotspot(40, 8, 53, 46) action ShowMenu("history")
        hotspot(96, 6, 46, 51) action ShowMenu('preferences')
        hotspot(149, 2, 39, 50) action ShowMenu('save')
        hotspot(197, 0, 45, 56) action MainMenu()
        hotspot(248, 1, 39, 55) action  Skip()

init python:
    config.overlay_screens.append("qq_menu")
...
screen save():
    ...
    imagemap:
        ...
        auto "images/Interface/save_%s.png"
        alpha True  
        hotspot (680, 372, 378, 231) clicked FileSave(1):
            use load_save_slot(number=1)
        hotspot (1205, 368, 378, 236) clicked FileSave(2):
            use load_save_slot(number=2)
        hotspot (680, 700, 379, 233) clicked FileSave(3):
            use load_save_slot(number=3)
        hotspot (1205, 703, 375, 229) clicked FileSave(4):
            use load_save_slot(number=4)

        hotspot (95, 87, 252, 54) action Return()

        hotspot (1699, 488, 32, 35) clicked FilePage(1)
        hotspot (1699, 547, 35, 33) clicked FilePage(2)
        hotspot (1701, 605, 34, 34) clicked FilePage(3)
        hotspot (1704, 665, 28, 35) clicked FilePage(4)
        hotspot (1699, 723, 33, 33) clicked FilePage(5)
        hotspot (1702, 783, 29, 32) clicked FilePage(6)

        hotspot (85, 642, 434, 56) action ShowMenu("preferences")
        hotspot (84, 714, 437, 50) action ShowMenu("load")
        hotspot (87, 780, 432, 50) action NullAction()
        hotspot (88, 850, 430, 51) action NullAction()

        hotspot (105, 951, 46, 44) action MainMenu()
        hotspot (157, 952, 45, 43) action Quit()
...
screen load():
    ...
    imagemap:
        ...
        auto "images/Interface/load_%s.png"
        ...
        hotspot (1099, 223, 71, 201) action FileDelete(1)
        hotspot (1099, 457, 73, 198) action FileDelete(2)
        hotspot (1101, 690, 69, 193) action FileDelete(3)

        hotspot (1698, 489, 45, 33) clicked FilePage(1)
        hotspot (1693, 550, 56, 28) clicked FilePage(2)
        hotspot (1702, 603, 32, 34) clicked FilePage(3)
        hotspot (1704, 666, 31, 36) clicked FilePage(4)
        hotspot (1705, 723, 29, 33) clicked FilePage(5)
        hotspot (1696, 783, 39, 34) clicked FilePage(6)


        hotspot (89, 631, 429, 73) action ShowMenu("preferences")
        hotspot (89, 715, 429, 51) action NullAction()
        hotspot (85, 786, 439, 43) action Show("save")
        hotspot (88, 848, 438, 53) action NullAction()
        hotspot (100, 948, 52, 52) action MainMenu()
        hotspot (159, 952, 39, 44) action Quit()
...
screen preferences():
    ...
    imagemap:
        auto "images/Interface/preferences_%s.png"
        alpha True
        hotspot (689, 507, 153, 31) action Preference('display', 'window')
        hotspot (689, 459, 250, 32) action Preference('display', 'fullscreen')

        hotspot (673, 493, 262, 36) action InvertSelected(Preference("transitions", "toggle"))
        hotspot (1210, 497, 244, 37) action Preference("after choices", "toggle")
        hotspot (1212, 547, 211, 31) action Preference("skip", "toggle")

        hotbar (1213, 790, 305, 15) value Preference('text speed')
        hotspot (1212, 812, 174, 32) action Preference("all mute", "enable")

        hotbar (688, 789, 305, 15) value Preference("music volume")
        hotbar (691, 843, 305, 15) value Preference("sound volume")

        hotspot (90, 82, 257, 58) action Return()

        hotspot (89, 631, 429, 73) action NullAction()
        hotspot (89, 715, 429, 51) action ShowMenu("load")
        hotspot (85, 786, 439, 43) action Show("save")
        hotspot (88, 848, 438, 53) action NullAction()
        hotspot (100, 948, 52, 52) action MainMenu()
        hotspot (159, 952, 39, 44) action Quit()

        hotspot (716, 748, 117, 46) action NullAction()
        hotspot (717, 811, 88, 35) action NullAction()
        hotspot (716, 865, 114, 27) action NullAction()
        hotspot (1244, 748, 223, 38) action NullAction()
...
style confirm_frame:
    background Frame([ "gui/confirm_frame.png", "images/Interface/Frame.png"], gui.confirm_frame_borders, tile=gui.frame_tile)
